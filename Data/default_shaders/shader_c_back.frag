...
void main(){
  ...
  vec4 p = cv * (1.-0.12*ga + 0.2*ga*ga);

  float val_r = p.x * abs(1. + 0.001*abs(v.x) / (gu+0.01 + var_1-gd) );
  float val_g = p.y * abs(1. + 0.001*abs(v.x) - 0.0001*val_r * (gu+0.01 + var_1-gd) );
  float val_b = p.z * abs(1. - 0.001*abs(v.x) * abs( 0.5 - pos.x ) );
  ...
  float fr = mix(c_o.x, val_r, 0.01*(1. - abs( 0.5 - pos.x )) );
  float fg = mix(c_o.y, val_g, 0.01*(1. - abs( 0.5 - pos.x )) );
  float fb = mix(c_o.z, val_b, 0.01*(1. - abs( 0.5 - pos.x )) );
