...
namespace ge {
  ...
  // Texture to FB ==============
  void draw_Text_to_FB(Texture * text, v2 & text_pos, v2 & text_size, FrameBuffer * fb_target, float z_level = 0, FrameShader * shader=nullptr){
    if(shader){
      shader->Bind();
      shader->UpdateUniforms();
    }
    fb_target->Target();
    text->Draw(text_pos, text_size, v2(0., 0.), v2(1., 1.), z_level);
    fb_target->Untarget();
    if(shader) shader->Unbind();
  };

  void draw_Text_to_Screen(Texture * text, v2 & text_pos, v2 & text_size, float z_level = 0, FrameShader * shader=nullptr){
    if(shader){
      shader->Bind();
      shader->UpdateUniforms();
    }
