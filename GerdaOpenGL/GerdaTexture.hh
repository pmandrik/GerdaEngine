...
namespace ge {
  ...
  class TextureGL : public Texture {
    ...
    TextureGL(std::shared_ptr<Image> img){
      ...
      atlas[ "" ] = TexTile(v2(0, 0), size, size);
    };
    

    virtual void Bind()  { glBindTexture(GL_TEXTURE_2D, id); }
  ...
  std::shared_ptr<TextureGL> load_texture_with_image(string file_name){
    /// simple interface to load RGBA PNG images
    std::shared_ptr<Image> img = std::make_shared<Image>();
    img->LoadPNG( file_name );
    std::shared_ptr<TextureGL> text = std::make_shared<TextureGL>( img );
    return text;
  }
